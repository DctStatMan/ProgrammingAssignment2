...
## c. Set (and cache) the inverse of the original matrix. Any matrix can be stored/cached by this function, but the 
##    intent is for the inverse matrix to be stored.
...
##    call it simply returns the inverted matrix from the cache
...
cacheSolve <- function(x, ...) {
    ...
    inverse <- x$getInverseMatrix()            #retrieve the inverted matrx, if any
    if(!is.null(inverse)) {
      message("Retrieving from cache.")
      return(inverse)
    }
    data <- x$get()                            #if there is no cached inverted matrix then create it 
    inverse <- solve(data)
